...
class RealtimeServiceImpl(
    ...
    private suspend fun ensureStarted() {
        ...
        startMutex.withLock {
            if (isStarted) {
                ...
                // compiler complains about something else
                return@withLock
            }
            startSseLoop()
            isStarted = true
        ...
        scope.launch {
            while (isActive) {
                try {
                    ...
                    client
                            .prepareGet("$baseUrl/api/realtime") {
            ...
            client.post("$baseUrl/api/realtime") {
                ...
                setBody(
                        buildJsonObject {
                                    ...
                                    put(
                                            "subscriptions",
                                            JsonArray(subscriptions.map { JsonPrimitive(it) })
                                    )
                                }
                                .toString()
                )
            ...
            if (sseEvent.event == topic || sseEvent.event == "*") {
                try {
                    ...
                    if (rConvId == conversationId &&
                                    (actionStr == "create" || actionStr == "update")
                    ) {
                        // Map safely
            ...
            if (sseEvent.event == topic || sseEvent.event == "*") {
                try {
                    ...
                    if (rConvId == conversationId) {
                        ...
                        val isTyping =
                                record["isTyping"]?.jsonPrimitive?.content?.toBoolean() ?: false
    ...
    override fun subscribeToReadReceipts(conversationId: String): Flow<Any> = flow {
        ensureStarted()
        ...
        eventFlow.collect { sseEvent ->
            if (sseEvent.event == topic || sseEvent.event == "*") {
                ...
                try {
                    ...
                    val record = data["record"]?.jsonObject
                    if (record != null) {
                        // Just emit the raw event for now as we don't have a model
                        emit(sseEvent)
                    }
                } catch (e: Exception) {
                    // ignore
    ...
    override suspend fun setTypingStatus(conversationId: String, isTyping: Boolean) {
        try {
            ...
            val records =
                    try {
                        pocketBase
                                .collection(DatabaseCollections.M_TYPING_STATUS)
                                .getList(
            ...
            if (existingId != null) {
                pocketBase.send<JsonObject>(
                        path =
                                "/api/collections/${DatabaseCollections.M_TYPING_STATUS}/records/$existingId",
                        method = "PATCH",
                        body = body.toString()
                )
            } else {
