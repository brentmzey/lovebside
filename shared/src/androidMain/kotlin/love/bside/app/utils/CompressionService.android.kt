...
actual object CompressionService {
    ...
    actual suspend fun compressToBase64(rawPayload: Option<String>): Option<String> {
        ...
        return try {
            ...
            Base64.encodeToString(compressedBytes, Base64.NO_WRAP).some()
        } catch (e: Exception) {
            println("⚠️ Android Compression failed: ${e.message}")
            none()
        }
    }
    ...
    actual suspend fun decompressFromBase64(base64Payload: Option<String>): Option<String> {
        ...
        return try {
            ...
            println("⚠️ Android Decompression failed, falling back to raw: ${e.message}")
            base64.some()
        }
    }
}
